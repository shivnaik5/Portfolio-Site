...
export const initConstellation = (root: ParentNode): (() => void) | undefined => {
  ...
  const label = root.querySelector<HTMLElement>('[data-constellation-label]');
  ...
  if (!canvas || !fallback || !hint || !label || !legend || !dataEl) return undefined;

  const groups: ConstellationGroup[] = JSON.parse(dataEl.textContent ?? '[]');
  if (!groups.length) return undefined;

  return mountConstellation(canvas, groups, { fallback, hint, label, legend });
};
